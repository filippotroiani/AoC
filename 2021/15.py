...
# extendCaveMap(grid) # AAA UNCOMMENT FOR PART 2
# printGrid(grid,(0,0))   # for debug purpose
start = grid[0][0]
start.weight = 0    # the start position risk will not be considered
goal = grid[-1][-1]
for row in grid:
    for element in row:
        element.findNeighbors(grid)
        element.h = heuristic(element, goal)

minPath = A_Star(grid, start, goal, heuristic)
if minPath is not None:
    # printGrid(grid,minPath[-1])   # for debug purpose
    print(f'The lowest total risk of any path through the cave is {goal.g}')
else:
    print('FAIL')