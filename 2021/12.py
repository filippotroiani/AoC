...
def visit(node, concatenatedLists, visited):
    ...
    if node not in visited:
        if node.islower(): visited.append(node)
    else:
        return 0
    count = 0
    for n in concatenatedLists[node]:
        count += visit(n, concatenatedLists, visited.copy())
    return count

def visit2(node, concatenatedLists, visited, twice = False):
    if node == 'end':   return 1
    if node not in visited:
        if node.islower(): visited.append(node)
    else:
    ...
    count = 0
    for n in concatenatedLists[node]:
        count += visit2(n, concatenatedLists, visited.copy(), twice)
    return count
...
print(f'The paths through the cave system that visit a single small cave twice are {numOfPaths2}.')
